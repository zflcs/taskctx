...
impl TlsArea {
    ...
    pub fn tls_ptr(&self) -> *mut u8 {
        unsafe { self.base.as_ptr().add(tp_offset(self.static_tls_size)) }
    }

    /// Allocates the memory region for TLS, and initializes it.
    pub fn alloc(start: usize, end: usize) -> Self {
        let static_tls_size = static_tls_size(start, end);
        let layout = Layout::from_size_align(tls_area_size(static_tls_size), TLS_ALIGN).unwrap();
        ...
        unsafe {
            ...
            // initialize TCB
            init_tcb(area_base, static_tls_size);
        }
...
fn static_tls_size(start: usize, end: usize) -> usize {
    let ans = align_up(end - start, TLS_ALIGN);
    log::info!(
...
fn tp_offset(static_tls_size: usize) -> usize {
    if cfg!(target_arch = "x86_64") {
        static_tls_size
    } else if cfg!(any(target_arch = "aarch64", target_arch = "riscv64")) {
        TCB_SIZE
...
fn tls_area_size(static_tls_size: usize) -> usize {
    if cfg!(target_arch = "x86_64") {
...
unsafe fn init_tcb(tls_area: *mut u8, static_tls_size: usize) {
    if cfg!(target_arch = "x86_64") {
        let tp_addr = tls_area.add(tp_offset(static_tls_size)).cast::<usize>();
